...
import { MoveRight, TextAlignJustify } from "lucide-react";
    ...
    >
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 py-4 md:px-10">
        <div className="flex items-center gap-3">
          ...
          {openMenu && (
            <div
              className="absolute top-0 left-0 w-60 h-full min-h-screen bg-black opacity-50 z-10 px-7 py-20 space-y-7"
              onClick={() => setOpenMenu(false)}
            >
              {drawerLinks.map((drawerLink, idx) => (
                <>
                  <div key={idx} className="flex justify-center items-center group">
                    <a href={drawerLink.href} className="flex-1">{drawerLink.label}</a>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--muted)] leading-relaxed">
                  Create eye-catching thumbnails with AI using text prompts and
                  reference images
                </p>
              </div>
            </div>
